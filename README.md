...
### 11. Diagnostický príkaz pre debugovanie host
Spustenie špeciálneho kontajnera s plným prístupom k host OS (len pre ladenie, nie produkcia).
...
### 12. Zoznam PersistentVolumes v namespace `ukf`
Zobrazenie všetkých PV v namespace (aj keď PV nie je viazaný priamo na namespace, zobrazuje sa v kontexte viazania cez PVC).
...
### 13. Zoznam PersistentVolumeClaims v namespace `ukf`
Zobrazenie všetkých PVC vytvorených v namespace `ukf`.
...
## Záver Kubernetes CLI

Tieto príkazy poskytujú ucelený základ pre prácu s Kubernetes cez `kubectl`. Pokrývajú nasadenie aplikácií, správu konfiguračných objektov, prístup do bežiacich kontajnerov a prácu s perzistentnými údajmi.
